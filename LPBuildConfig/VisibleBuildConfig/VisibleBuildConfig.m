...
+ (instancetype)sharedInstance
{
    ...
    dispatch_once(&onceToken, ^{
        instance = [[VisibleBuildConfig alloc] init];
    });
    return instance;
}
...
-(instancetype)init
{
    self = [super init];
    if (self) {
        [self setupBuildConfigDataFromPlist];
...
-(void)setupBuildConfigDataFromPlist
{
    NSString *path = [[NSBundle mainBundle] pathForResource:@"VisibleBuildConfig" ofType:@"plist"];
